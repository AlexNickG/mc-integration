...
public class LocationService {
    ...
    private void parseAreas(AreaModel area, Integer countryId, List<CityDto> allCitiesForCountry) {
        ...
        if (subAreas.isEmpty()) {// если регион не имеет подрегионов, то это - город
            area.setParentId(String.valueOf(countryId));
            allCitiesForCountry.add(cityMapper.hhApiToCityDto(area));
        } else {// если регион имеет подрегионы, то рекурсивно идем до городов
            for (AreaModel sub : subAreas) {
                parseAreas(sub, countryId, allCitiesForCountry);
            }
        }
    }
}
